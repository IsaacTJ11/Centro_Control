...
function updateActiveStates() {
    ...
    // Limpiar clases pre-activas si quedaron de alguna navegación anterior
    ...
    // Detectar qué submenú necesita estar abierto para el destino
    ...
    // Colapsar solo los submenús que NO son el destino (evita el flash al navegar dentro del mismo grupo)
    ...
    document.querySelectorAll('.submenu-arrow.rotated').forEach(arrow => {
        ...
        if (parentItem !== targetParentItem) {
    ...
    fetch(page, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        ...
        .then(html => {
            ...
            Promise.all(cssPromises).then(() => {
                ...
                const SKIP_SCRIPTS = ['main.js', 'cdn.jsdelivr.net', 'cdnjs.cloudflare.com', 'fonts.googleapis'];

                doc.querySelectorAll('script[src]').forEach(script => {
                    const src = script.getAttribute('src');
                    if (!src) return;
                    // Saltar scripts globales/CDN: sólo recargar scripts de página
...
function initPageScripts(page) {
    // Limpiar intervalos anteriores de cualquier página
...
function updateActiveStates() {
    const path = window.location.pathname;
    // Buscamos el item que coincida con la URL actual
    const currentItem = document.querySelector(`.submenu-item[data-page="${path}"], .sidebar-item[data-page="${path}"]`);
    ...
    if (currentItem) {
        ...
        // SI ES UN HIJO: Buscamos al padre y lo activamos también
        if (currentItem.classList.contains('submenu-item')) {
            ...
            if (submenu) {
                ...
                if (parentItem) {
                    // ESTO evita el salto a Home: El padre se pone naranja de inmediato
                    parentItem.classList.add('active'); 
                    
                    // Solo desplegamos el acordeón si el menú está ABIERTO
                    if (!sidebar.classList.contains('collapsed')) {
                        submenu.classList.add('show');
