...
def background_updater():
    """Ejecuta actualización automática cada 3 minutos"""
    time.sleep(10)  # Esperar a que Flask levante antes del primer intento
    while True:
        update_wind_data()
        time.sleep(300)
...
@app.route('/reportes_rer')
def reportes_rer():
    # Intentamos actualizar (esto ahora tomará solo 3s si falla la red)
    update_wind_data()

    # Si hay un error Y no tenemos ningún dato previo en cache, mostramos la página 500
    if wind_data_cache['error'] is not None and not wind_data_cache['data']['wayra_i']:
        if es_error_conexion(wind_data_cache['error']):
            return render_template('pagina500.html', 
                                   error_code=503,
                                   error_detail="Error de conexión con la base de datos de los equipos RER."), 503
    ...
    # Si llegamos aquí, o la actualización fue exitosa, o falló pero tenemos datos viejos que mostrar
    wayra_i_data = wind_data_cache['data'].get('wayra_i', [])
    wayra_ext_data = wind_data_cache['data'].get('wayra_ext', [])
    
    # Unificar listas para filtrar
    todos = wayra_i_data + wayra_ext_data
    
    aerogeneradores_falla = [aero for aero in todos if aero.get('tipo') in ['FALLA', 'STOP', 'PAUSA']]
    aerogeneradores_mantenimiento = [aero for aero in todos if aero.get('tipo') == 'MANT']
    ...
    return render_template(
        ...
        aerogeneradores_mantenimiento=aerogeneradores_mantenimiento,
        last_update=wind_data_cache['last_update'],
        hay_error=wind_data_cache['error'] is not None # Opcional: para mostrar un banner de "Datos desactualizados"
    )
